...
本ツールは、自身の目的で、自身の環境のために作成したもので、特定の環境や、特定のデータでの動作を確認しております。
...
どうしてもうまくゆかない場合は、問い合わせフォームからそのファイルをお送りください。

・実行中にエラーが発生する
読み込ませたファイルと実行ファイルのディレクトリに作成されるログファイル（年月日時分秒のファイル）を問い合わせフォームからお送りください。
...
レーザー加工の場合は、移動速度を遅めにしたほうが良い結果になります。
これは、移動開始や停止は遅いため、その部分だけ余計にレーザーが当たることになり、その部分が余計に焼けたり切断されるためです。移動速度が遅いと、この差が比較的するなくなり、結果も良好になります。

その他、加工のノウハウや実例は、NCToolのホームページやYouTubeで公開しています。
